...
hmm = GaussianHMM(
    n_components=n_components, n_iter=n_iter, init_params="kmeans", tol=0.001
)
...
def game():
    ...
    while True:
        ...
        if len(observation_sequence) >= 10:
            observation_sequence_reshaped = observation_sequence.reshape(-1, 1).astype(
                np.float64
            )
            hmm.fit(observation_sequence_reshaped)

        if len(observation_sequence) >= 10:
            next_observation = hmm.predict(
                observation_sequence.reshape(-1, 1).astype(np.float64)
            )[-1]
        else:
